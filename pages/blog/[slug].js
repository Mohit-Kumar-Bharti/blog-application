...
function BlogPage({
  ...
  useEffect(() => {
    axios
      .get("https://learnwithharsh.herokuapp.com/posts")
      .then((res) => {
        const post = res.data.find(
          (post) => post.postname === router.query.slug
        );
  ...
  function handleLikes(e) {
    ...
    if (postid !== 0) {
      axios
        .post(
          `https://learnwithharsh.herokuapp.com/posts/update/${postid}`,
          updatedlike
        )
    ...
    <Layout title={title} description={excerpt}>
      <div className={styles.mainDiv}>
          ...
          <div className={styles.likesDiv}>
            <div>
              <IconContext.Provider
                value={{ className: `${styles.likesIcon}` }}
              >
        ...
        <div className={styles.author}>
          <div className={styles.aboutAndImage}>
            <img src={author_image} alt="" className={styles.authorImage} />
            <h4>{author}</h4>
          </div>
      ...
      <div className={styles.linksDiv}>
        ...
        <Link href={`/blog/category/${category.toLowerCase()}`}>
          <a className={styles.goBack}>Read more blogs on {category}</a>
        </Link>
      </div>
    </Layout>
  );
